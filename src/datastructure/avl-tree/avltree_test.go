...
func TestDeleteExistentItemWithRightRotationBFLeftSubtreeZero(t *testing.T) {
	tree := AVlTree{}
	tree.insert(6)
	tree.insert(7)
	tree.insert(4)
	...
	tree.delete(7)
	root := tree.root
	if root.height != 2 || root.right.value != 6 {
		t.Errorf("Expected height to be %d but found %d", 2, root.height)
	}
}
...
func TestDeleteExistentItemWithBigTree(t *testing.T) {
	tree := AVlTree{}
	tree.insert(1)
	tree.insert(2)
	tree.insert(3)
	...
	tree.delete(6)
	root := tree.root
	if root.height != 3 || root.right.value != 9 {
		t.Errorf("Expected height to be %d but found %d", 1, root.height)
	}
}
